// Contract selection for trader-mimicry - holds for cfg.HOLDING_PERIOD_DAYS (5 trading
// days, ~1 calendar week), so needs an expiration comfortably beyond that, unlike the ORB
// bot (same-day) or overnight-drift (next-day). Filters to the nearest expiration that is
// at least cfg.MIN_EXPIRATION_DAYS_OUT calendar days out, so the full hold has real time
// value left even if the position needs to run past the target hold length.
...
async function getMinDaysOutChain(symbol, direction) {
  ...
  const eligible = all.filter((c) => daysFromToday(c.expiration_date) >= cfg.MIN_EXPIRATION_DAYS_OUT);
  if (eligible.length === 0) return [];
  const nearestEligible = eligible.reduce(
    (min, c) => (c.expiration_date < min ? c.expiration_date : min),
    eligible[0].expiration_date
  );
  return eligible
    .filter((c) => c.expiration_date === nearestEligible)
...
async function selectStructure(symbol, direction, underlyingPrice, budgetUsd, getQuote) {
  const chain = await getMinDaysOutChain(symbol, direction);
  if (chain.length === 0) return { structure: null, reason: `no chain >= ${cfg.MIN_EXPIRATION_DAYS_OUT}d out` };
...
module.exports = { getMinDaysOutChain, selectStructure, getLatestOptionQuote, todayET };
